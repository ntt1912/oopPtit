...
public class StaticAccountClass {
    ...
    static class BankAccount {
        ...
        BankAccount(String name, BigInteger balance) {
            this.id = total + 1;
            this.name = name;
    ...
    public static void main(String[] args) {
        ...
        while (t-- > 0) {
            BankAccount.reset();
            List<BankAccount> list_Account = new ArrayList<>();
            String[] input = scanner.nextLine().split(" ");
            String tmp = "";
            for (int i = 1; i < input.length; i++) {
                try {
                    BigInteger balance_i = new BigInteger(input[i]);
                    list_Account.add(new BankAccount(tmp.trim(), balance_i));
                    tmp = "";
                } catch (Exception e) {
                    tmp = tmp + " " + input[i];
                }
                ...
                switch (input_request[i]) {
                    ...
                    case "chuyen":
                        ...
                        if (id_m - 1 < total_c && id_s - 1 < total_c) {
                            if (list_Account.get(id_m - 1).withdraw(amount_c)) {
                                list_Account.get(id_s - 1).deposit(amount_c);
                            }
                        }
                        break;
                    default:
            ...
            for (BankAccount i : list_Account) {
